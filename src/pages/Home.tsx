...
export default function Home() {
  ...
  return (
    <div>
      {/* Search + Filters area — button row, then filter row below (full width) */}
      <div ref={filterRef} className="mb-6 sm:mb-8">
        <div className="flex items-center gap-2 sm:gap-3">
          <SearchBar value={query} onChange={setQuery} />
          <button
            onClick={() => setShowFilters((prev) => !prev)}
            className={`flex items-center gap-1.5 sm:gap-2 text-sm font-medium px-3 sm:px-4 py-2.5 rounded-lg transition-colors shrink-0 ${
